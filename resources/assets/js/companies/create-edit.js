...
function loadCreateEditCompanyData() {
        ...
        if ($('#editAdminEmployerDescriptionQuillData').length) {
            window.editAdminEmployerDescriptionQuill = new Quill(
                '#editAdminEmployerDescriptionQuillData', {
                    ...
                    placeholder: Lang.get('messages.employer_menu.enter_employer_details'),
                    theme: 'snow',
                });
    ...
    if ($('#addIndustryDescriptionQuillData').length) {
        window.industry = new Quill('#addIndustryDescriptionQuillData', {
            placeholder: Lang.get('messages.employer_menu.enter_industry_details'),
            modules: {
                toolbar: [
                    ['bold', 'italic', 'underline', 'strike'],
                    ['clean'],
    ...
    if($('#ownershipDescription').length){
        window.ownership = new Quill('#ownershipDescription', {
            placeholder: Lang.get('messages.employer_menu.enter_ownership_details'),
            modules: {
                toolbar: [
                    ['bold', 'italic', 'underline', 'strike'],
                    ['clean'],
       ...
       if (!$('#editEmployeeDetails').length) {
           ...
           if (isEdit) {
               ...
               if (editAdminEmployerDescriptionQuill.getText().trim().length ===
                   0) {
                   displayErrorMessage(Lang.get('messages.employer_menu.employer_details_field'));
                   e.preventDefault();
        ...
        if (!urlValidation(facebookUrl, facebookExp)) {
            displayErrorMessage(Lang.get('messages.employer_menu.valid_facebook_url'));
            $('#btnSave').prop('disabled', false);
            return false;
        }
        if (!urlValidation(twitterUrl, twitterExp)) {
            displayErrorMessage(Lang.get('messages.employer_menu.valid_twitter_url'));
            $('#btnSave').prop('disabled', false);
            return false;
        }
        if (!urlValidation(googlePlusUrl, googlePlusExp)) {
            displayErrorMessage(Lang.get('messages.employer_menu.valid_google_plus_url'));
            $('#btnSave').prop('disabled', false);
            return false;
        }
        if (!urlValidation(linkedInUrl, linkedInExp)) {
            displayErrorMessage(Lang.get('messages.employer_menu.valid_linkedin_url'));
            $('#btnSave').prop('disabled', false);
            return false;
        }
        if (!urlValidation(pinterestUrl, pinterestExp)) {
            displayErrorMessage(Lang.get('messages.employer_menu.valid_pinterest_url'));
            $('#btnSave').prop('disabled', false);
            return false;
        }
       return true;
